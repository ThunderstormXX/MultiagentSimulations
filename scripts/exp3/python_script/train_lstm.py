...
class LSTMTrainer:
    ...
    def prepare_data(self, simulation_results):
        X, y = [], []
        for i in range(15, len(simulation_results)):  # Longer history for more agents
            features = []
            for j in range(i-15, i):
                ...
                total_orders = sum([row.get(f'random_{k}_orders', 0) for k in range(1, 16)])
                avg_random_pnl = np.mean([row.get(f'random_{k}_pnl', 0.0) for k in range(1, 16)])
                market_volatility = abs(mid - 100.0) / 100.0
                step_normalized = row.get('step', 0) / 1000.0
                
                features.append([
                    bid / 100.0, ask / 100.0, spread / 10.0, mid / 100.0,
                    avg_random_pnl / 1000.0, total_orders / 100.0, 
                    market_volatility, step_normalized
                ])
            ...
            price_change = max(-0.05, min(0.05, price_change))  # Smaller range for stability
    ...
    def train(self, X, y, epochs=40):
        ...
        dataloader = DataLoader(dataset, batch_size=64, shuffle=True)  # Larger batch for more data
        
        # Better initialization for larger network
        ...
        for epoch in range(epochs):
            ...
            for batch_X, batch_y in dataloader:
                ...
                loss.backward()
                torch.nn.utils.clip_grad_norm_(list(self.model.parameters()) + list(self.fc.parameters()), 0.5)
                self.optimizer.step()
...
def run_training_simulations(num_simulations=15):
    all_results = []
    
    for i in range(num_simulations):
        print(f"Running training simulation {i+1}/{num_simulations}")
        
        # Create 10 random agents for training (smaller than full experiment)
        agents = [RandomAgent(f"random_{j}", 10000) for j in range(1, 11)]
        
        sim = MarketSimulation(agents)
        sim.run(800)  # Longer simulations for better training data
...
def main():
    ...
    print("Starting LSTM training for large-scale experiment...")
    ...
    print(f"Training on {len(combined_X)} samples with enhanced features...")
    ...
    print("Large-scale LSTM training completed!")
