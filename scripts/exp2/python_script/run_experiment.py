...
def create_agents(config):
    ...
    for i in range(config['agents']['rl_agents']):
        agents.append(RLAgent(f"rl_{i+1}", config['agents']['initial_cash']))
    
    return agents

def main():
    print("Starting Multi-Agent Experiment 2...")
    ...
    if config['output']['save_plots']:
        ...
        plot_pnl(results_df, agent_ids, os.path.join(results_dir, 'baseline_pnl_plot.png'))
        plot_market_dynamics(results_df, os.path.join(results_dir, 'baseline_market_dynamics.png'))
    
    # Print final results
    print("Baseline simulation completed!")
    print("\nFinal PnL by agent type:")
    
    random_pnl = [agent.pnl for agent in agents if 'random' in agent.agent_id]
    rl_pnl = [agent.pnl for agent in agents if 'rl' in agent.agent_id]
    
    print(f"Random agents: {[f'{pnl:.2f}' for pnl in random_pnl]}")
    print(f"RL agents: {[f'{pnl:.2f}' for pnl in rl_pnl]}")
    print(f"Random avg: {sum(random_pnl)/len(random_pnl):.2f}")
    print(f"RL avg: {sum(rl_pnl)/len(rl_pnl):.2f}")
